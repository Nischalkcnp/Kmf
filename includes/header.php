...
<body class="bg-gray-50 text-gray-800 antialiased">
    <header class="bg-kmf-blue/95 backdrop-blur-md text-white shadow-xl sticky top-0 z-50 border-b border-white/5">
            ...
            <div class="flex items-center justify-between h-20 md:h-24 transition-all duration-300">
                <a href="<?php echo BASE_URL; ?>index.php" class="flex items-center gap-3 md:gap-4 group">
                    ...
                    <div class="flex flex-col justify-center">
                        <span class="font-extrabold text-lg md:text-xl lg:text-2xl leading-none tracking-tight font-montserrat text-white group-hover:text-kmf-orange-light transition-colors">KMF NEPAL</span>
                        <span class="font-medium text-[10px] md:text-xs tracking-[0.2em] uppercase text-gray-300 mt-1">Foundational Impact</span>
                    </div>
                </a>
        ...
        <!-- Mobile Menu -->
        <div id="mobile-menu" class="fixed inset-0 z-[60] bg-kmf-blue-dark lg:hidden transition-transform duration-500 translate-x-full">
            <div class="flex flex-col h-full p-8 pt-24">
                <nav class="flex flex-col gap-4 overflow-y-auto pb-12">
                <?php foreach ($topPages as $page): 
                    if ($page['slug'] === 'donate' || $page['slug'] === 'donate.php') continue;
                ...
                <div class="flex flex-col">
                    ...
                    <?php if ($hasSubmenu): ?>
                    <div class="mt-4 ml-4 flex flex-col gap-3 border-l-2 border-white/10 pl-6">
                        <?php foreach ($subPages as $sub): 
                            $subUrl = BASE_URL . 'view.php?slug=' . urlencode($sub['slug']);
                            if (file_exists(ROOT_PATH . $sub['slug'] . '.php')) {
                                $subUrl = BASE_URL . $sub['slug'] . '.php';
    ...
    <script>
        ...
        document.querySelectorAll('.nav-link-animate').forEach(link => {
            link.addEventListener('click', function(e) {
                // Prevent immediate navigation to see animation (optional, but better for feel)
