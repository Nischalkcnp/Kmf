...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && validateCsrf()) {
    ...
    if ($id) {
        $stmt = $pdo->prepare("UPDATE strategic_areas SET title=?, slug=?, excerpt=?, content=?, icon=?, sort_order=?, is_active=? WHERE id=?");
        $stmt->execute([$title, $slug, $excerpt, $content, $icon, $sort, $active, $id]);
    } else {
        $stmt = $pdo->prepare("INSERT INTO strategic_areas (title, slug, excerpt, content, icon, sort_order, is_active) VALUES (?,?,?,?,?,?,?)");
        $stmt->execute([$title, $slug, $excerpt, $content, $icon, $sort, $active]);
    }
...
<form method="post" class="bg-white rounded-lg shadow p-6 mb-6 max-w-3xl">
    ...
    <div class="space-y-4">
        ...
        <div class="flex gap-4"><div><label class="block text-sm font-medium mb-1">Icon (education|people|health)</label><input type="text" name="icon" value="<?php echo escape($edit['icon']); ?>" class="w-full px-4 py-2 border rounded-lg"></div><div><label class="block text-sm font-medium mb-1">Sort</label><input type="number" name="sort_order" value="<?php echo (int)$edit['sort_order']; ?>" class="w-full px-4 py-2 border rounded-lg"></div></div>
...
<table class="bg-white rounded-lg shadow overflow-hidden w-full max-w-3xl">
    ...
    <tbody>
        ...
        <tr class="border-t"><td class="p-3"><?php echo escape($a['title']); ?></td><td class="p-3"><?php echo escape($a['slug']); ?></td><td class="p-3"><?php echo (int)$a['sort_order']; ?></td><td class="p-3"><a href="?edit=<?php echo $a['id']; ?>" class="text-kmf-orange hover:underline">Edit</a></td></tr>
        <?php endforeach; ?>
    </tbody>
</table>
